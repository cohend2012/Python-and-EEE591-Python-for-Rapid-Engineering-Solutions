...
list_of_number=range(2,10001,1,) # list of numbers from 2 to 10,000
prime_number_list = [] # store the prime numbers
...
for number_under_evaluation in list_of_number: # looking at each number in the list of number
    ...
    for potential_factor in list_of_number: # looking at each potential_factor
        #print(potential_factor)

        if (potential_factor or number_under_evaluation) != 0: # do not go on if either number is 0

            top_number_to_check = np.sqrt(number_under_evaluation) # find the sqrt(n), top number to check
